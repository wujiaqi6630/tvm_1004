...
struct SoftmaxMaxAttrs : public tvm::AttrsNode<SoftmaxMaxAttrs> {
  int axis;

  TVM_DECLARE_ATTRS(SoftmaxMaxAttrs, "relay.attrs.SoftmaxMaxAttrs") {
    TVM_ATTR_FIELD(axis).set_default(-1)
      .describe("The axis to sum over when computing softmax_max.");
  }
};

struct SoftmaxSumAttrs : public tvm::AttrsNode<SoftmaxSumAttrs> {
  int axis;

  TVM_DECLARE_ATTRS(SoftmaxSumAttrs, "relay.attrs.SoftmaxSumAttrs") {
    TVM_ATTR_FIELD(axis).set_default(-1)
      .describe("The axis to sum over when computing softmax_sum.");
  }
};

struct SoftmaxDivAttrs : public tvm::AttrsNode<SoftmaxDivAttrs> {
  int axis;

  TVM_DECLARE_ATTRS(SoftmaxDivAttrs, "relay.attrs.SoftmaxDivAttrs") {
    TVM_ATTR_FIELD(axis).set_default(-1)
      .describe("The axis to sum over when computing softmax_div.");
  }
};
...
/*! \brief Attributes for LRN_sqr operator */
struct LRN_sqrAttrs : public tvm::AttrsNode<LRN_sqrAttrs> {
  int size;
  int axis;

  TVM_DECLARE_ATTRS(LRN_sqrAttrs, "relay.attrs.LRN_sqrAttrs") {
    TVM_ATTR_FIELD(size).set_default(5)
      .describe("The size of the local region to be considered for normalization.");
    TVM_ATTR_FIELD(axis).set_default(1)
      .describe("Axis of input data layout channel.");
...
/*! \brief Attributes for LRN_pow operator */
struct LRN_powAttrs : public tvm::AttrsNode<LRN_powAttrs> {
  int size;
  int axis;
  double bias;
  double alpha;
  ...
  TVM_DECLARE_ATTRS(LRN_powAttrs, "relay.attrs.LRN_powAttrs") {
    TVM_ATTR_FIELD(size).set_default(5)
      .describe("The size of the local region to be considered for normalization.");
    TVM_ATTR_FIELD(axis).set_default(1)
      .describe("Axis of input data layout channel.");
...
/*! \brief Attributes for LRN_div operator */
struct LRN_divAttrs : public tvm::AttrsNode<LRN_divAttrs> {
  int axis;

  TVM_DECLARE_ATTRS(LRN_divAttrs, "relay.attrs.LRN_divAttrs") {
    TVM_ATTR_FIELD(axis).set_default(1)
      .describe("Axis of input data layout channel.");
  }
};
