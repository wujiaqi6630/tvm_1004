...
inline Tensor lrn_sqr(const Tensor& data,
									int size,
									int axis = 1,
									std::string name = "tensor",
									std::string tag = kBroadcast) {
...
inline Tensor lrn_pow(const Tensor& data,
									int size,
									int axis = 1,
									float alpha = 0.0001,
									float beta = 0.75,
	...
	auto sqrt_sum_up = tvm::compute(
			...
			[&](Var i, Var j, Var k, Var l) {
				return tvm::pow(bias +
												(div(alpha * data(i, j, k, l), size)),
												beta);
			});
	return sqrt_sum_up;
}

inline Tensor lrn_div(const Tensor& pow_data,
									const Tensor& in_data,
									int axis = 1,
									std::string name = "tensor",
									std::string tag = kBroadcast) {
	CHECK_EQ(in_data->shape.size(), 4) << "LRN requires 4-D input";
	CHECK(axis == 1 || axis == 3) << "axis should be 1 or 3 for NCHW and NHWC";
	
	return topi::divide(in_data, pow_data);
}
