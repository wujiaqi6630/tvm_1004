...
def conv2d_nchw(Input, Filter, stride, padding, dilation, out_dtype=None):
    ...
    """
    #"""
    output_data = lambda n, oc, oh, ow: tvm.sum(
        tvm.expr.Select(
            ...
            Input[n, ic, oh * stride_h + kh * dilation_h - pad_top, ow * stride_w + kw * dilation_w - pad_left].astype(out_dtype) * Filter[oc, ic, kh, kw].astype(out_dtype),
            0.0 * Filter[oc, ic, kh, kw].astype(out_dtype)),
        axis=[ic, kh, kw])
    #"""
    """
    output_data = lambda n, oc, oh, ow: tvm.sum(
            Input[n, ic, tvm.min(in_height+pad_top-1, tvm.max(oh*stride_h+kh*dilation_h,pad_top))-pad_top, tvm.min(in_width+pad_left-1, tvm.max(pad_left, ow*stride_w+kw*dilation_w))-pad_left].astype(out_dtype)
            * Filter[oc, ic, kh, kw].astype(out_dtype),
        axis=[ic, kh, kw])
    """
    return tvm.compute(
...
def schedule_conv2d(cfg, outs):
    ...
    def traverse(op):
        ...
        if 'conv2d_nchw' in op.tag:
            bn, oc, oh, ow = op.axis
            s[op].pragma(bn,"SIMD")
            s[op].pragma(bn,"unroll", tag="DPU")
            s[op].pragma(oc,"loop_split(oc,3,*:blockIdx_z,4:threadIdx_z,1:local)")
            s[op].pragma(oc,"unroll", tag="DPU")
            s[op].pragma(oh,"loop_split(oh,3,*:blockIdx_y,4:threadIdx_y,7:local)")
            s[op].pragma(oh,"unroll", tag="DPU")
            s[op].pragma(ow,"loop_split(ow,3,*:blockIdx_x,4:threadIdx_x,7:local)")
            s[op].pragma(ow,"unroll", tag="DPU")
