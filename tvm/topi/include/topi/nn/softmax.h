...
inline Tensor softmaxMax(const Tensor &x,
											int axis = -1,
											std::string name = "tensor",
											std::string tag = "softmax_output") {
	auto input_shape = x->shape;
	auto ndim = input_shape.size();
	CHECK_EQ(ndim, 2) << "SoftmaxMax op requires 2-D input";
  if (axis < 0) {
		axis = ndim + axis;
	}
	CHECK_LT(axis, ndim) << "axis parameter should be less than input dim";
	
	auto ok = tvm::reduce_axis(Range(0, input_shape[axis]), "ok");
  Array<PrimExpr> eval_range = {input_shape[0]};

	auto maxelem = tvm::compute(eval_range,
                           [&](Var i) {return topi::MaxOp(x(i, ok),{ok});
	});

	return maxelem;
}

inline Tensor softmaxSum(const Tensor &max_data,
  												const Tensor &in_data,
											int axis = -1,
											std::string name = "tensor",
											std::string tag = "softmax_output") {
	auto input_shape = in_data->shape;
	auto ndim = input_shape.size();
	CHECK_EQ(ndim, 2) << "SoftmaxSum op requires 2-D input";
	if (axis < 0) {
		axis = ndim + axis;
	}
	CHECK_LT(axis, ndim) << "axis parameter should be less than input dim";
	...
	auto expsum = tvm::compute(eval_range,
													 [&](Var i) {return tvm::sum(
													 		tvm::exp(in_data(i, ok)-max_data(i))
													 		,{ok});
	});
...
inline Tensor softmaxDiv(const Tensor &sum_data,
													const Tensor &max_data,
													const Tensor &in_data,
											int axis = -1,
											std::string name = "tensor",
											std::string tag = "softmax_output") {
	...
	auto divelem = tvm::compute(input_shape,
													 [&](Var i, Var j) {return tvm::exp(in_data(i, j)-max_data(i))/sum_data(i);
	});
