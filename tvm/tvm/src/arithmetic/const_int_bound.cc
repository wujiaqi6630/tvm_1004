...
class ConstIntBoundAnalyzer::Impl :
 ...
 public:
  ...
  //wjq 2020/09/18
  Entry VisitExpr_(const PowNode* op) final {
    Entry a = VisitExpr(op->a);
    Entry b = VisitExpr(op->b);
    return BinaryOpBoundry(a, b, InfAwarePow);
  }
  //wjq 2020/09/18
 ...
 private:
  ...
  //wjq 2020/09/18
  static int64_t InfAwarePow(int64_t x, int64_t y) {
    if (!WillOverflow<PowNode>(x, y, kNegInf, kPosInf)) return x ^ y;
    if ((x > 0 && y > 0) || (x < 0 && y < 0)) return kPosInf;
    return kNegInf;
  }
  //wjq 2020/09/18
