...
class TypeInferencer : private ExprFunctor<Type(const Expr&)>,
 ...
 private:
  ...
  Type Unify(const Type& t1, const Type& t2, const ObjectRef& expr) {
    try {
      ...
      this->ReportFatalError(
        expr, RELAY_ERROR("Error unifying `" << t1 << "` and `" << t2 << "`: " << e.what()));
      return Type();
    }
  }
  ...
  Type VisitExpr_(const GlobalVarNode* op) final {
    ...
    if (!mod_.defined()) {
      this->ReportFatalError(GetRef<GlobalVar>(op),
        RELAY_ERROR("Cannot do type inference on global variables " \
          "without a module"));
    }
  ...
  void VisitPattern_(const PatternConstructorNode* con, const Type& t) {
    ...
    if (td->type_vars.size() != tc->args.size()) {
      this->ReportFatalError(pc, 
																	RELAY_ERROR("The number of type args must match"
  ...
  Type VisitExpr_(const IfNode* ite) final {
    ...
    this->Unify(cond_type,TensorTypeNode::Scalar(tvm::DataType::Bool()),ite->cond);
  ...
  Type PrimitiveCall(const FuncTypeNode* op,Array<Type> arg_types,const Attrs& attrs,
    ...
    solver_.AddConstraint(TypeRelationNode::make(rel->func, arg_types, arg_types.size() - 1, attrs), loc);
    return rtype;
  }
  ...
  FuncType InstantiateFuncType(const FuncTypeNode* fn_ty, const Array<Type>& ty_args) {
    ...
    Type inst_ty = FuncTypeNode::make(fn_ty->arg_types,ret_type, {},fn_ty->type_constraints);
  ...
  Type GeneralCall(const CallNode* call, Array<Type> arg_types) {
    ...
    if (fn_ty_node == nullptr && inc_ty_node == nullptr) {
      this->ReportFatalError(
        GetRef<Call>(call),
        RELAY_ERROR("only expressions with function types can be called, found "<< ftype));
    }
    ...
    for (auto cs : fn_ty->type_constraints) {
      if (const auto* tr = cs.as<TypeRelationNode>()) {
        solver_.AddConstraint(TypeRelationNode::make(tr->func, tr->args, tr->num_inputs, call->attrs),
          GetRef<Call>(call));
      } else {
  ...
  Type VisitExpr_(const RefCreateNode* op) final {return RefTypeNode::make(GetType(op->value));}
  ...
  Type VisitExpr_(const ConstructorNode* c) final {
    ...
    return FuncTypeNode::make(c->inputs, TypeCallNode::make(c->belong_to, types),td->type_vars, {});
  }
...
class TypeInferencer::Resolver : public ExprMutator, PatternMutator {
 public:
  ...
  Expr AttachCheckedType(const T* op) {
    ...
    bool need_update_fn =(std::is_base_of<FunctionNode, T>::value && update_missing_type_annotation_ &&
    ...
    if (!new_e.unique()) {
      ...
      new_fn = (std::is_base_of<FunctionNode, T>::value ?
...
Function InferType(const Function& func,const Module& mod,const GlobalVar& var) {
...
Pass InferType() {
  runtime::TypedPackedFunc<Function(Function, Module, PassContext)> pass_func =
    [=](Function f, Module m, PassContext pc) {return Downcast<Function>(InferType(f, m));};
