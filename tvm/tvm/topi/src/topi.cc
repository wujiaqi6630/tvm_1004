...
#include <topi/dpu/bnn.h>
#include <topi/dpu/default.h>
#include <topi/dpu/injective.h>
...
TVM_REGISTER_GLOBAL("topi.nn.softmaxMax")
.set_body([](TVMArgs args, TVMRetValue *rv) {
*rv = nn::softmaxMax(args[0], args[1]);
});

TVM_REGISTER_GLOBAL("topi.nn.softmaxSum")
.set_body([](TVMArgs args, TVMRetValue *rv) {
*rv = nn::softmaxSum(args[0], args[1], args[2]);
});

TVM_REGISTER_GLOBAL("topi.nn.softmaxDiv")
.set_body([](TVMArgs args, TVMRetValue *rv) {
*rv = nn::softmaxDiv(args[0], args[1], args[2], args[3]);
});
...
TVM_REGISTER_GLOBAL("topi.nn.lrn_sqr")
.set_body([](TVMArgs args, TVMRetValue *rv) {
	*rv = nn::lrn_sqr(args[0], args[1], args[2]);
	});

TVM_REGISTER_GLOBAL("topi.nn.lrn_pow")
.set_body([](TVMArgs args, TVMRetValue *rv) {
	*rv = nn::lrn_pow(args[0], args[1], args[2],
								static_cast<double>(args[3]),
								static_cast<double>(args[4]),
								static_cast<double>(args[5]));
	});

TVM_REGISTER_GLOBAL("topi.nn.lrn_div")
.set_body([](TVMArgs args, TVMRetValue *rv) {
	*rv = nn::lrn_div(args[0], args[1], args[2]);
	});
...
/* dpu schedules */
TVM_REGISTER_GLOBAL("topi.dpu.schedule_binarize_pack")
.set_body([](TVMArgs args, TVMRetValue *rv) {
  *rv = topi::dpu::schedule_binarize_pack(args[0], args[1]);
  });

TVM_REGISTER_GLOBAL("topi.dpu.schedule_binary_dense")
.set_body([](TVMArgs args, TVMRetValue *rv) {
  *rv = topi::dpu::schedule_binary_dense(args[0], args[1]);
  });

TVM_REGISTER_GLOBAL("topi.dpu.default_schedule")
.set_body([](TVMArgs args, TVMRetValue *rv) {
  if (args[2]) {
    *rv = topi::dpu::default_schedule_auto_inline(args[0], args[1]);
  } else {
    *rv = topi::dpu::default_schedule(args[0], args[1]);
  }
  });

TVM_REGISTER_GLOBAL("topi.dpu.schedule_injective")
.set_body([](TVMArgs args, TVMRetValue *rv) {
  *rv = topi::dpu::schedule_injective(args[0], args[1]);
  });

TVM_REGISTER_GLOBAL("topi.dpu.schedule_injective_from_existing")
.set_body([](TVMArgs args, TVMRetValue *rv) {
  *rv = topi::dpu::schedule_injective_from_existing(args[0], args[1]);
  });
...
.register_func({ "dpu" }, WrapSchedule(topi::dpu::schedule_injective));
...
.register_func({ "dpu" }, WrapSchedule(topi::dpu::default_schedule));
...
.register_func({ "dpu" }, WrapScheduleFromExisting(topi::dpu::schedule_injective_from_existing));
