...
@reg.register_compute("nn.relu")
def compute_relu(attrs, inputs, outputs, target):
    # Compute definition of relu
    #layout = attrs.layout
    out_dtype = outputs.dtype
    #assert layout in ["NCHW", "NHWC", "NCHW4c", "HWCN"]
    assert len(inputs) >= 1
    out = topi.nn.relu(inputs[0], out_dtype)
    return [out]

@reg.register_schedule("nn.relu")
def schedule_relu(attrs, outs, target):
    # Schedule definition of relu op
    #layout = attrs.layout
    with target:
        return topi.generic.schedule_relu(outs)
...
'''
# dropout
#reg.register_schedule("nn.dropout", schedule_injective)
@reg.register_compute("nn.dropout")
def compute_dropout(attrs, inputs, outputs, target):
    # Compute definition of dropout
    #layout = attrs.layout
    out_dtype = outputs.dtype
    #assert layout in ["NCHW", "NHWC", "NCHW4c", "HWCN"]
    assert len(inputs) >= 1
    out = topi.nn.dropout(inputs[0], out_dtype)
    return [out]

@reg.register_schedule("nn.dropout")
def schedule_dropout(attrs, outs, target):
    # Schedule definition of dropout op
    layout = attrs.layout
    with target:
        return topi.generic.schedule_dropout(outs,layout)


reg.register_pattern("nn.dropout", OpPattern.ELEMWISE)
...
def compute_softmax(attrs, inputs, outputs, target):
    ...
    out = topi.nn.softmax(inputs[0], axis)
    return [out]
...
#reg.register_pattern("nn.softmax", OpPattern.ELEMWISE)
# wjq 2020/09/21

#softmaxMax
@reg.register_compute("nn.softmaxMax")
def compute_softmaxMax(attrs, inputs, outputs, target):
    # Compute definition of softmaxMax
    assert len(inputs) == 1
    assert len(inputs[0].shape) == 2
    axis = attrs.axis
 
    out = topi.nn.softmaxMax(inputs[0], axis)
    return [out]

@reg.register_schedule("nn.softmaxMax")
def schedule_softmaxMax(_, outputs, target):
    #Schedule definition of softmaxMax
    with target:
        return topi.generic.schedule_softmaxMax(outputs)

reg.register_pattern("nn.softmaxMax", OpPattern.OPAQUE)

#softmaxSum
@reg.register_compute("nn.softmaxSum")
def compute_softmaxSum(attrs, inputs, outputs, target):
    # Compute definition of softmaxSum
    assert len(inputs) == 2
    assert len(inputs[0].shape) == 2
    axis = attrs.axis
 
    out = topi.nn.softmaxSum(inputs[0], inputs[1], axis)
    return [out]

@reg.register_schedule("nn.softmaxSum")
def schedule_softmaxSum(_, outputs, target):
    #Schedule definition of softmaxSum
    with target:
        return topi.generic.schedule_softmaxSum(outputs)

reg.register_pattern("nn.softmaxSum", OpPattern.OPAQUE)

#softmaxDiv
@reg.register_compute("nn.softmaxDiv")
def compute_softmaxDiv(attrs, inputs, outputs, target):
    # Compute definition of softmaxDiv
    assert len(inputs) == 3
    assert len(inputs[0].shape) == 2
    axis = attrs.axis
 
    out = topi.nn.softmaxDiv(inputs[0], inputs[1], inputs[2], axis)
    return [out]

@reg.register_schedule("nn.softmaxDiv")
def schedule_softmaxDiv(_, outputs, target):
    #Schedule definition of softmaxDiv
    with target:
        return topi.generic.schedule_softmaxDiv(outputs)

reg.register_pattern("nn.softmaxDiv", OpPattern.OPAQUE)
...
def compute_bias_add(attrs, inputs, outputs, target):
    # Compute definition of bias_add
    #layout = attrs.layout
    assert len(inputs) == 2
    axis = attrs.axis
    out_dtype = outputs.dtype
    ...
    #bias = inputs[1]
    #assert layout in ["NCHW", "NHWC", "NCHW4c", "HWCN"]
    out = topi.nn.bias_add(inputs[0], inputs[1], axis, out_dtype)
    return [out]

@reg.register_schedule("nn.bias_add")
def schedule_bias_add(attrs, outs, target):
    # Schedule definition of bias_add op
    #layout = attrs.layout
    with target:
        return topi.generic.schedule_bias_add(outs)
...
# max_pool2d
@reg.register_compute("nn.max_pool2d")
def compute_max_pool2d(attrs, inputs, outputs, target):
    ...
    assert layout in ["NCHW", "NHWC", "NCHW4c", "NCHW8c","HWCN"]
    #if groups == 1:
    assert len(inputs) >= 1
    out = topi.nn.max_pool2d(inputs[0], pool_size, strides, padding, layout, out_dtype)
...
# lrn_sqr
@reg.register_compute("nn.lrn_sqr")
def compute_lrn_sqr(attrs, inputs, out_dtype, target):
    #Compute definition of lrn_sqr
    assert len(inputs) == 1
    return [topi.nn.lrn_sqr(inputs[0], attrs.size, attrs.axis)]


@reg.register_schedule("nn.lrn_sqr")
def schedule_lrn_sqr(attrs, outs, target):
    #Schedule definition of lrn_sqr
    with target:
        return topi.generic.schedule_lrn_sqr(outs)

reg.register_pattern("nn.lrn_sqr", OpPattern.OPAQUE)

# lrn_pow
@reg.register_compute("nn.lrn_pow")
def compute_lrn_pow(attrs, inputs, out_dtype, target):
    #Compute definition of lrn_pow
    assert len(inputs) == 1
    return [topi.nn.lrn_pow(inputs[0], attrs.size, attrs.axis,
                        attrs.alpha, attrs.beta, attrs.bias)]

@reg.register_schedule("nn.lrn_pow")
def schedule_lrn_pow(attrs, outs, target):
    #Schedule definition of lrn_pow
    with target:
        return topi.generic.schedule_lrn_pow(outs)

reg.register_pattern("nn.lrn_pow", OpPattern.OPAQUE)

# lrn_div
@reg.register_compute("nn.lrn_div")
def compute_lrn_div(attrs, inputs, out_dtype, target):
    #Compute definition of lrn_div
    assert len(inputs) == 2
    return [topi.nn.lrn_div(inputs[0], inputs[1], attrs.axis)]

@reg.register_schedule("nn.lrn_div")
def schedule_lrn_div(attrs, outs, target):
    #Schedule definition of lrn_div
    with target:
        return topi.generic.schedule_lrn_div(outs)

reg.register_pattern("nn.lrn_div", OpPattern.OPAQUE)
