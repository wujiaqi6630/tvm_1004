...
class OperatorConverter(object):
    ...
    def __init__(self, init_layer_dict, predict_layer, exp_tab):
        ...
        self.convert_map = {
            ...
            "LRN_SPLITE1": self.convert_lrn_sqr,
            "LRN_SPLITE2": self.convert_lrn_pow,
            "LRN_SPLITE3": self.convert_lrn_div,
            ...
            "SoftmaxMax": self.convert_softmaxMax,
            "SoftmaxSum": self.convert_softmaxSum,
            "SoftmaxDiv": self.convert_softmaxDiv,
    ...
    def convert_softmaxMax(self, op):
        #Convert softmaxMax layer
        params = dict()
        softmax_param = op.softmax_param
        ...
        out = _op.nn.softmaxMax(in_expr, **params)

        return out

    def convert_softmaxSum(self, max_op, op):
        # Convert SoftmaxSum layer
        params = dict()
        softmax_param = op.softmax_param
        params["axis"] = softmax_param.axis
        ...
        #max_shape = [batch]
        #max_value = np.asarray(max_op.data, np.float32)
        #max_value = np.reshape(max_value, max_shape)
        #max_expr = self.exp_tab.new_const(max_value, dtype="float32")
        
        #out = _op.nn.softmaxSum(max_expr, in_expr, **params)
        out = _op.nn.softmaxSum(max_op, in_expr, **params)
        
        return out

    def convert_softmaxDiv(self, max_op, sum_op, op):
        #Convert SoftmaxDiv layer
        params = dict()
        softmax_param = op.softmax_param
        params["axis"] = softmax_param.axis
        ...
        out = _op.nn.softmaxDiv(sum_op, max_op, in_expr, **params)
    ...
    def convert_pooling(self, op):
        ...
        global SOFTMAX_FLAG
        if LRN_FLAG:
            input_name = 'lrn_div'+str(LRN_INDEX-1)
            LRN_FLAG = 0
        else:
            inputs = op.bottom
            input_name = inputs[0]
    ...
    def convert_lrn(self, op):
        ...
        params["bias"] = lrn_params.k # avoid div op appear 0
    ...
    def convert_lrn_sqr(self, op):
        ...
        input_name = inputs[0]
        in_expr = self.exp_tab.get_expr(input_name)
        out = _op.nn.lrn_sqr(in_expr, **params)
        return out

    def convert_lrn_pow(self, sqr_op, op):
        params = dict()
        lrn_params = op.lrn_param
        params["size"] = lrn_params.local_size
        ...
        #sqr_expr = sqr_op.args[0]
        out = _op.nn.lrn_pow(sqr_op, **params)
        return out

    def convert_lrn_div(self,  pow_op, op):

        inputs = op.bottom
        input_name = inputs[0]
        in_expr = self.exp_tab.get_expr(input_name)
        #pow_expr = pow_op.args[0]
        
        out = _op.nn.lrn_div(pow_op, in_expr)
        return out
    ...
    def convert_op_to_relay(self):
        ...
        def caffe_setExpr_splite(self, output_tensors, ret):
            self.exp_tab.set_expr(output_tensors, ret)


        def caffe_setExpr(self, output_tensors, ret):
        ...
        for pl in self.predict_layer:
            ...
            #'''
            if op_type == "LRN" and OPT_LEVEL == -1:
                #lrn_sqr
                output_tensors = 'lrn_sqr'+str(LRN_INDEX)
                ret1 = self.convert_map["LRN_SPLITE1"](pl)
                caffe_setExpr_splite(self, output_tensors, ret1)
       
                #lrn_pow
                output_tensors = 'lrn_pow'+str(LRN_INDEX)
                ret2 = self.convert_map["LRN_SPLITE2"](ret1, pl)
                caffe_setExpr_splite(self, output_tensors, ret2)
                
                #lrn_div
                output_tensors = 'lrn_div'+str(LRN_INDEX)
                ret3 = self.convert_map["LRN_SPLITE3"](ret2, pl)
                caffe_setExpr_splite(self, output_tensors, ret3)
                
                LRN_FLAG = 1
                LRN_INDEX = LRN_INDEX+1
                continue
            #'''
            if op_type == "Softmax" and OPT_LEVEL == -1:
                #softmaxMax
                output_tensors = 'softmaxMax'+str(SOFTMAX_INDEX)
                ret1 = self.convert_map["SoftmaxMax"](pl)
                caffe_setExpr_splite(self, output_tensors, ret1)
       
                #softmaxSum
                output_tensors = 'softmaxSum'+str(SOFTMAX_INDEX)
                ret2 = self.convert_map["SoftmaxSum"](ret1, pl)
                caffe_setExpr_splite(self, output_tensors, ret2)
                
                #softmaxDiv
                output_tensors = 'prob'
                ret3 = self.convert_map["SoftmaxDiv"](ret1, ret2, pl)
                caffe_setExpr_splite(self, output_tensors, ret3)
                
                #SOFTMAX_FLAG = 1
                #SOFTMAX_INDEX = SOFTMAX_INDEX+1
                continue
            #'''
            ret = self.convert_map[op_type](pl)
            caffe_setExpr(self, output_tensors, ret)
...
def from_caffe(init_net, predict_net, shape_dict, dtype_dict):
    ...
    for n in model_outputs:
        ...
        #if OPT_LEVEL == -1:
        #    outputs.append(exp_tab.get_expr('softmaxDiv1'))
        #else:
        outputs.append(exp_tab.get_expr(n))
    outputs = outputs[0] if len(outputs) == 1 else _expr.Tuple(outputs)
    ###wjq###
    func = relay.Function(analysis.free_vars(outputs), outputs)
