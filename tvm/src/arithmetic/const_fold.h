...
//wjq 2020/09/18
template<>
inline PrimExpr TryConstFold<ir::PowNode>(PrimExpr a, PrimExpr b) {
  TVM_ARITH_CONST_PROPAGATION({
      const DataType& rtype = a.dtype();
			CHECK(rtype.is_float()) << "power only applies to float";
			/*
      if (pa && pb) return IntImmNode::make(rtype, pow(pa->value, pb->value));
      if (pa) {
        if (pa->value == 1) return b;
        if (pa->value == 0) return a;
      }
      ...
      */
      if (fa && fb) return FloatImmNode::make(rtype, std::pow(fa->value, fb->value));
      if (fa) {
        if (fa->value == 1) return b;
        if (fa->value == 0) return a;
      }
