...
//wjq 2020/09/18
PrimExpr RewriteSimplifier::Impl::
VisitExpr_(const PowNode* op) {
  PrimExpr ret = IRMutatorWithAnalyzer::VisitExpr_(op);
  op = ret.as<PowNode>();
  PrimExpr const_res = TryConstFold<PowNode>(op->a, op->b);
  if (const_res.defined()) return const_res;
  // Pattern var to match any expression
  PVar<PrimExpr> x, y, z, b1, b2, s1, s2;
  // Pattern var match IntImm
  ...
  // Vector rules
  if (op->dtype.lanes() != 1) {
    TVM_TRY_REWRITE(pow(broadcast(x, lanes), broadcast(y, lanes)),
                    broadcast(pow(x , y), lanes));
    TVM_TRY_REWRITE(pow(ramp(b1, s1, lanes) , broadcast(x, lanes)),
                    ramp(pow(b1, x), pow(s1, x), lanes));
    //TVM_TRY_REWRITE(pow(broadcast(x, lanes) * ramp(b1, s1, lanes)),
    //                ramp(pow(b1, x), pow(s1, x), lanes));
  }

  if (IsIndexType(op->dtype)) {
    // constant simplification rule
    //TVM_TRY_REWRITE(pow(x,(c1+c2)), pow(x, c1) * pow(x, c2));
    //TVM_TRY_REWRITE(pow(x,(c1-c2)), pow(x, c1) / pow(x, c2));
    //TVM_TRY_REWRITE(pow(x,(c1*c2)), pow((x, c1) ,c2));

    // canonicalization
    //TVM_TRY_RECURSIVE_REWRITE(pow(x,c1)*pow(x,c2)), pow(x,c2)*pow(x,c1)));
  }
  return ret;
}
//wjq 2020/09/18
