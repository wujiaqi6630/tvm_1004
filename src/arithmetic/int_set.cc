...
//wjq 2020/09/17
template<>
inline IntervalSet Combine<ir::PowNode>(Analyzer* analyzer,
																				IntervalSet a,
																				IntervalSet b) {
	if (a->IsSinglePoint() && b->IsSinglePoint()) {
		return IntervalSet::SinglePoint(a->min_value * b->min_value);
	...
	if (b->IsSinglePoint()) {
		...
		if (is_one(b->min_value)) return a;
		if (analyzer->CanProveGreaterEqual(b->min_value, 0)) {
			PrimExpr min_value = a->HasLowerBound() ? pow(a->min_value , b->min_value) : neg_inf();
			PrimExpr max_value = a->HasUpperBound() ? pow(a->max_value , b->min_value) : pos_inf();
			return IntervalSet(min_value, max_value);
		} else if (analyzer->CanProveGreaterEqual(-b->min_value, 1)) {
			PrimExpr min_value = a->HasUpperBound() ? pow(a->max_value , b->min_value) : neg_inf();
			PrimExpr max_value = a->HasLowerBound() ? pow(a->min_value , b->min_value) : pos_inf();
			return IntervalSet(min_value, max_value);
		} else if (a->HasUpperBound() && a->HasLowerBound()) {
			using ir::SelectNode;
			PrimExpr sign = b->min_value >= make_zero(b->min_value.dtype().element_of());
			PrimExpr e1 = pow(a->min_value ,  b->min_value);
			PrimExpr e2 = pow(a->max_value ,  b->min_value);
			return IntervalSet(SelectNode::make(sign, e1, e2), SelectNode::make(sign, e2, e1));
		}
	}
	DLOG(WARNING) << "Return Everything in CombineInterval Pow";
	return IntervalSet::Everything();
}
...
//wjq 2020/09/17
  IntervalSet VisitExpr_(const PowNode* op) final {
	  return VisitBinaryExpr_(op);
  }
//wjq 2020/09/17
