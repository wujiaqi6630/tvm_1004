...
inline void PrintConst(const FloatImmNode* op, std::ostream& os, CodeGenC* p) { // NOLINT(*)
  switch (op->dtype.bits()) {
    case 64: case 32: {
      std::ostringstream temp;
      /*********wjq**********/
			/* \brief determines the form of data representation
			 *  e.g. (float)0: 
			 *        use std::scientific type："0.00000000e+0f"
			 *        use std::defaultfloat type: "0"
			 */
      //temp << std::scientific << op->value;
			temp << std::fixed << op->value;
...
//wjq 2020/09/18
void CodeGenC::VisitExpr_(const PowNode* op, std::ostream& os) {  // NOLINT(*)
  PrintBinaryExpr(op, "powf", os, this);
}
//wjq 2020/09/18
